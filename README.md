...
| 채팅 | 노트북 단위 채팅 이력 조회, 질문 전송, 답변 근거 확인 |
...
AI 답변에는 `근거 N` 버튼이 표시됩니다.
버튼을 열면 FastAPI가 검색하고 Spring이 저장한 reference chunk를 확인할 수 있습니다.
...
| --- | --- |
| 문서명 | 근거가 나온 PDF 제목 |
| 섹션명 | chunk 주변 제목 또는 섹션 정보 |
| 페이지 | PDF 페이지 번호 |
| 문서 청크 | 문서 전체 기준 chunk 순서 |
| 페이지 청크 | 해당 페이지 안에서의 chunk 순서 |
| 본문 | 실제 검색된 근거 chunk 내용 |

이 구조 덕분에 사용자는 AI 답변이 어떤 문서와 페이지를 근거로 생성됐는지 새로고침 후에도 다시 확인할 수 있습니다.

현재 일부 PDF는 추출 텍스트의 공백이 붙어 보일 수 있습니다.
이는 프론트 렌더링 문제가 아니라 PDF 텍스트 추출/전처리 단계의 개선 여지입니다.
...
- 노트북 생성 후 PDF를 업로드하고 문서 상태가 `COMPLETED`로 바뀌는지 확인합니다.
- 문서 내용을 바탕으로 질문하면 AI 답변과 함께 `근거 N` 버튼이 표시되는지 확인합니다.
- 근거 카드에 문서명, 섹션명, 페이지, 청크 위치가 표시되는지 확인합니다.
- 새로고침 후에도 이전 AI 답변의 근거가 유지되는지 확인합니다.
